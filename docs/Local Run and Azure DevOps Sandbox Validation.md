...
`ado.organization` is mandatory when `ado.http-client-enabled=true`; startup fails clearly if it is missing or blank. The organization comes from configuration, not from the webhook payload.

Each `ado.projects` key must match the Azure DevOps project name from the webhook exactly. If the project name contains spaces, dots, or special characters, use Spring Boot YAML bracket notation so the map key is preserved:
...
* Confirm the configured `ado.projects` key matches the webhook project name exactly, including spaces, dots, and capitalization.
* Use bracket notation for project names with spaces or special characters, such as `"[Project Name With Spaces]"`.
...
* Confirm logged ADO paths contain encoded spaces such as `%20`, not double-encoded values such as `%2520`.

ADO read returns `FAILED_RETRYABLE`:

* Retryable read failures include 429, 5xx, and transport failures from Azure DevOps.
* Retrying the same payload is valid if the event was not marked processed.

ADO read returns `FAILED_NON_RETRYABLE`:

* Non-retryable read failures include cases such as 400, 401, 403, or 404 from Azure DevOps.
* Correct the config, PAT permission, project name, Work Item id, or revision before retrying.
...
* The idempotency key is `project + workItemId + revision`.
* Repeating the same revision after `COMPLETED` or `SKIPPED` is skipped.
* To retest locally, create a new Azure DevOps revision or remove the local SQLite file.
