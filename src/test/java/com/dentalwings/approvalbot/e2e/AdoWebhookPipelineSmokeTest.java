...
@SpringBootTest(properties = {
        "ado.organization=org", "ado.personal-access-token=test-token",
        "ado.projects.ProjectA.enabled=true", "ado.projects.ProjectA.supported-work-item-types[0]=Test Case",
        ...
        "ado.projects.ProjectA.approvals.sqa-users[0]=sqa@example.com", "bot.identity-email=bot@example.com",
        "webhook.shared-secret.value=test-webhook-secret", "idempotency.type=in-memory", "idempotency.ttl-hours=24",
...
class AdoWebhookPipelineSmokeTest
{
    ...
    void happyPathProcessesWebhookThroughAdoFetchPatchAndComment() throws Exception
    {
        ...
        mockMvc.perform(webhookPost().contentType(MediaType.APPLICATION_JSON)
                        .content(webhookPayload(701, 31, "Human User", "human@example.com"))).andExpect(status().isAccepted())
        ...
        assertThat(patchBody.getFirst()).containsEntry("op", "test").containsEntry("path", "/rev")
                .containsEntry("value", 31);
        assertThat(patchBody).anySatisfy(operation -> assertThat(operation).containsEntry("op", "replace")
                .containsEntry("path", "/fields/System.State").containsEntry("value", "In Review"));
        assertThat(patchBody).anySatisfy(operation -> assertThat(operation).containsEntry("op", "replace")
                .containsEntry("path", "/fields/Custom.ApprovedBySME").containsEntry("value", ""));
        assertThat(patchBody).anySatisfy(operation -> assertThat(operation).containsEntry("op", "replace")
                .containsEntry("path", "/fields/Custom.ApprovedBySQA").containsEntry("value", ""));
        ...
        assertThat(commentBody.get("text").toString()).contains("returned to In Review")
    ...
    void patchFailureDoesNotCreateCommentOrRetryPatch() throws Exception
    {
        ...
        mockMvc.perform(webhookPost().contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(status().isServiceUnavailable()).andExpect(jsonPath("$.status").value("FAILED_RETRYABLE"));
    ...
    void commentFailureAfterSuccessfulPatchCompletesWithWarningWithoutRetriesOrRollback() throws Exception
    {
        ...
        mockMvc.perform(webhookPost().contentType(MediaType.APPLICATION_JSON)
                        .content(webhookPayload(703, 51, "Human User", "human@example.com"))).andExpect(status().isAccepted())
    ...
    {
        mockMvc.perform(webhookPost().contentType(MediaType.APPLICATION_JSON)
                        .content(webhookPayload(704, 61, "Approval Bot", "bot@example.com"))).andExpect(status().isAccepted())
    ...
    void duplicateWebhookIsIdempotentlySkippedWithoutSecondAdoCall() throws Exception
    {
        ...
        mockMvc.perform(webhookPost().contentType(MediaType.APPLICATION_JSON).content(payload))
        ...
        mockMvc.perform(webhookPost().contentType(MediaType.APPLICATION_JSON).content(payload))
                .andExpect(status().isAccepted()).andExpect(jsonPath("$.status").value("SKIPPED"));
    ...
    static class RecordingAdoExchange implements ExchangeFunction
    {
        ...
        {
            return requests.stream().filter(request -> request.method() == HttpMethod.GET).toList();
        }

        List<RecordedRequest> patchRequests()
        {
            return requests.stream().filter(request -> request.method() == HttpMethod.PATCH).toList();
        }

        List<RecordedRequest> commentRequests()
        {
            return requests.stream().filter(request -> request.method() == HttpMethod.POST).toList();
        }

        @Override
        public Mono<ClientResponse> exchange(ClientRequest request)
        {
            requests.add(new RecordedRequest(request.method(), request.url().toString(), request.headers(),
                    captureBody(request)));
            var response = responses.pollFirst();
            if (response == null)
            {
                return Mono.error(
                        new AssertionError("Unexpected ADO HTTP call: " + request.method() + " " + request.url()));
            }
            return Mono.just(ClientResponse.create(response.status())
                    .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE).body(response.body()).build());
        }
