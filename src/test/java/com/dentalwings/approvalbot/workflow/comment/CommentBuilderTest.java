...
class CommentBuilderTest {
    ...
    void approvedAutomaticallyCommentIncludesSmeAndSqaApprovalValues() {
        var comment = commentBuilder.approvedAutomatically(
    ...
    void proposedChangesCommentIncludesVerifiedDisplayNameAndEmailWhenEmailExists() {
        var comment = commentBuilder.proposedChanges(
                new Identity("John Doe", "john.doe@company.com"),
                List.of(new ChangedField("System.Title", "Old", "New"))
        );

        assertThat(comment).contains("Proposed changes by John Doe [john.doe@company.com](mailto:john.doe@company.com).");
    ...
    void proposedChangesCommentListsChangedFieldsDeterministically() {
        var comment = commentBuilder.proposedChanges(
                new Identity("John Doe", "john.doe@company.com"),
                List.of(
                        new ChangedField("System.Title", "Old title", "New title"),
                        new ChangedField("Microsoft.VSTS.TCM.Steps", "Old steps", "New steps")
                )
        );

        assertThat(comment).containsSubsequence(
    ...
    void invalidApprovedCorrectionCommentMatchesV1Wording() {
        assertThat(commentBuilder.invalidApprovedCorrection())
                .isEqualTo("The Test Case was returned to In Review because it does not have valid SME and SQA approvals from different users.");
    }
    ...
    void skippedOrNoOpDecisionReturnsNoComment() {
        ...
        assertThat(commentBuilder.fromDecision(WorkflowDecision.completed(List.of(), "Comment should not be emitted", "No patch"))).isEmpty();
    }
    ...
    void commentBuilderDoesNotDependOnHttpOrAdoClientClasses() {
        ...
        assertThat(commentBuilder.fromDecision(WorkflowDecision.completed(
                List.of(PatchOperation.replaceField("System.State", "Approved")),
                "Existing workflow comment",
                "Completed"
        ))).contains("Existing workflow comment");
    }
}
