...
class AzureDevOpsConfigDiscoveryServiceTest
{
    ...
    void validateProjectUsesReadOnlyProjectEndpointAndBasicAuthWithoutExposingPatInUrl()
    {
        ...
        assertThat(exchange.requests.getFirst().url().toString())
                .isEqualTo(
                        "https://dev.azure.com/STMN-Group/_apis/projects/ADOnis%202.0%20Test%20Project?api-version=7.1")
    ...
    void listWorkItemTypesReturnsNamesFromAdoResponse()
    {
        ...
        assertThat(exchange.requests.get(0).url().toString())
                .isEqualTo(
                        "https://dev.azure.com/STMN-Group/_apis/projects/ADOnis%202.0%20Test%20Project?api-version=7.1")
                .contains("ADOnis%202.0%20Test%20Project").doesNotContain("%24expand", "%2520");
        assertThat(exchange.requests.get(1).url().toString()).isEqualTo(
                "https://dev.azure.com/STMN-Group/_apis/projects/project-id-1/properties?keys=System.ProcessTemplateType,System.CurrentProcessTemplateId,System.Process%20Template&api-version=7.1-preview.1");
        assertThat(exchange.requests.get(2).url().toString()).isEqualTo(
                        "https://dev.azure.com/STMN-Group/_apis/work/processes/process-id-1/workitemtypes?api-version=7.1")
    ...
    void listWorkItemTypeOptionsMapsAdoValueArrayToSelectorOptions()
    {
        ...
        assertThat(exchange.requests).extracting(request -> request.url().toString())
                .noneMatch(url -> url.contains("/_apis/wit/workitemtypes?")).noneMatch(url -> url.contains("/fields"));
    }
    ...
    @ParameterizedTest
    @CsvSource({ "BAD_REQUEST,failed with status 400", "FORBIDDEN,authorization failed", "NOT_FOUND,not found" })
    ...
    {
        var service = new AzureDevOpsConfigDiscoveryService("secret-pat",
        ...
        assertThat(result.status()).isEqualTo(ConfigValidationStatus.ERROR);
        assertThat(result.message()).contains("transport error").contains("IllegalStateException")
                .doesNotContain("secret-pat");
    }
    ...
    {
        var service = new AzureDevOpsConfigDiscoveryService("secret-pat", request -> {
            ...
            if (request.url().toString().contains("/properties?"))
            {
                return Mono.just(ClientResponse.create(HttpStatus.OK)
                        .header(HttpHeaders.CONTENT_TYPE, "application/json").body("""
                                {"value":[{"name":"System.CurrentProcessTemplateId","value":"process-id-1"}]}
                                """).build());
            }
            return Mono.just(
                    ClientResponse.create(HttpStatus.OK).header(HttpHeaders.CONTENT_TYPE, "application/json").body("""
                            {"id":"project-id-1","name":"ADOnis 2.0 Test Project"}
                            """).build());
        }, new AzureDevOpsUrlBuilder());
        ...
        assertThat(result.status()).isEqualTo(ConfigValidationStatus.ERROR);
        assertThat(result.message()).contains("transport error").contains("DataBufferLimitException").contains("262144")
                .doesNotContain("secret-pat");
    }
    ...
    {
        var exchange = new RecordingExchangeFunction(List.of(new RecordedResponse("""
                {"id":"project-id-1","name":"ADOnis 2.0 Test Project"}
                """, HttpStatus.OK), new RecordedResponse("""
                {"value":[
                ...
                """, HttpStatus.NOT_FOUND), new RecordedResponse("""
    ...
    void identitySearchMapsUniqueNameToSelectableNormalizedOption()
    {
        ...
        assertThat(exchange.requests.getFirst().url().toString()).contains("filterValue=sme")
    ...
    void repeatedNormalizedIdentitySearchUsesBackendCacheWithoutCallingAdoAgain()
    {
        ...
        assertThat(second.diagnostics()).containsEntry("backendCacheHit", true).containsEntry("adoRequestCount", 1L);
        assertThat(exchange.requests).hasSize(1);
    ...
    void expiredIdentitySearchCacheCausesNewAdoRequest()
    {
        ...
        var exchange = new RecordingExchangeFunction("""
                {"value":[{"displayName":"Rene Example","uniqueName":"rene@example.test"}]}
                """, HttpStatus.OK);
        var service = new AzureDevOpsConfigDiscoveryService("secret-pat", exchange, new AzureDevOpsUrlBuilder(), cache);
    ...
    void identitySearchCacheStoresOnlyNormalizedSelectorOptions()
    {
        ...
        cache.put("STMN-Group", "Sandbox", "rene",
                ConfigLookupResult.valid(List.of(new ConfigSelectorOption("rene@example.test", "Rene Example",
                        "rene@example.test", "ADO", "aad.user-1"))));
    ...
    void projectCandidateCacheSupportsNewQueriesWithoutMoreAdoCalls()
    {
        ...
        assertThat(exchange.requests).hasSize(3);
        assertThat(second.diagnostics()).containsEntry("candidatePoolCacheHit", true).containsEntry("candidatePoolSize",
                3);
    }
    ...
    void projectCandidateCacheUsesCanonicalOrganizationAndProjectId()
    {
        ...
        var first = new ProjectIdentityCandidateCache.ProjectIdentityCandidates(true, "project-id-1", "scp.1",
                "project-scope", List.of(new ConfigSelectorOption("first@example.test", "First", "first@example.test",
                "project-scope")));
        var refreshed = new ProjectIdentityCandidateCache.ProjectIdentityCandidates(true, "project-id-1", "scp.1",
                "project-scope", List.of(new ConfigSelectorOption("second@example.test", "Second",
                "second@example.test", "project-scope")));
        ...
        assertThat(cache.get("STMN-GROUP", "Project Name").orElseThrow().values())
                .extracting(ConfigSelectorOption::value).containsExactly("second@example.test");
    }
    ...
    void singleProjectPoolMatchAvoidsGraphSubjectQuery()
    {
        ...
        assertThat(result.diagnostics()).containsEntry("projectPoolMatchCount", 1)
                .containsEntry("graphFallbackAttempted", false).containsEntry("sourceSelected", "project-scope");
        assertThat(exchange.requests).hasSize(3);
    }
    ...
    void emptyGraphResultIsNegativeCachedForSameAndExtendedQueries()
    {
        ...
        assertThat(repeated.diagnostics()).containsEntry("graphFallbackAttempted", false)
                .containsEntry("graphNegativeCacheHit", true);
        assertThat(extended.diagnostics()).containsEntry("graphFallbackAttempted", false)
                .containsEntry("graphNegativeCacheHit", true);
    ...
    void graphNegativeCacheDoesNotSuppressProjectPoolMatches()
    {
        ...
        var exchange = projectIdentityExchange("""
                ...
                """);
        var service = new AzureDevOpsConfigDiscoveryService("secret-pat", exchange, new AzureDevOpsUrlBuilder(),
        ...
        assertThat(result.values()).extracting(ConfigSelectorOption::value).containsExactly("rene@example.test");
        assertThat(result.diagnostics()).containsEntry("graphNegativeCacheHit", false)
    ...
    {
        var cache = new IdentitySearchResultCache(Duration.ofMinutes(5), 10, Clock.systemUTC());
        cache.put("STMN-Group", "Project A", "rene", ConfigLookupResult.valid(
                List.of(new ConfigSelectorOption("rene@example.test", "Rene", "rene@example.test", "project-scope"))));
    ...
    void sparseProjectCandidatesFallBackToOfficialGraphSubjectQuery()
    {
        ...
        assertThat(exchange.requests.getLast().url().toString())
                .contains("/_apis/graph/subjectquery?api-version=7.1-preview.1");
    }
    ...
    void graphSubjectQueryObjectRootMapsAllResolvableUsersAndFallsBackToLoginForDisplay()
    {
        ...
        assertThat(result.status()).isEqualTo(ConfigValidationStatus.VALID);
        assertThat(result.values()).extracting(ConfigSelectorOption::value).containsExactly("rene.one@example.test",
                "rene.two@example.test");
    ...
    {
        var exchange = new RecordingExchangeFunction(
                List.of(new RecordedResponse("png-binary", HttpStatus.OK, "image/png;api-version=7.1")));
        var service = discovery(exchange);
        ...
        assertThat(exchange.requests).hasSize(1);
        assertThat(exchange.requests.getFirst().url().toString())
                .contains("/avatars?size=small&format=png&api-version=7.1");
    }
    ...
    void failedAvatarLoadIsCachedBrieflyWithoutAffectingIdentityDiscovery()
    {
        ...
        var exchange = new RecordingExchangeFunction(
                List.of(new RecordedResponse("not found", HttpStatus.NOT_FOUND, "application/json")));
        var service = new AzureDevOpsConfigDiscoveryService("secret-pat", exchange, new AzureDevOpsUrlBuilder(),
                new IdentitySearchResultCache(), new ProjectIdentityCandidateCache(), avatarCache);
    ...
    {
        var exchange = new RecordingExchangeFunction(
                List.of(new RecordedResponse("{\"value\":\"not-an-image\"}", HttpStatus.OK, "application/json")));
        var service = discovery(exchange);
    ...
    private RecordingExchangeFunction processDiscoveryExchange(String processWorkItemTypesBody,
            HttpStatus processStatus)
    {
        return new RecordingExchangeFunction(List.of(new RecordedResponse("""
                {"id":"project-id-1","name":"ADOnis 2.0 Test Project"}
                """, HttpStatus.OK), new RecordedResponse("""
                {"value":[{"name":"System.CurrentProcessTemplateId","value":"process-id-1"}]}
                """, HttpStatus.OK), new RecordedResponse(processWorkItemTypesBody, processStatus)));
    }

    private RecordingExchangeFunction projectIdentityExchange(String graphUsersBody)
    {
        return new RecordingExchangeFunction(
                List.of(new RecordedResponse("{\"id\":\"project-id-1\",\"name\":\"Sandbox\"}", HttpStatus.OK),
    ...
    private static class RecordingExchangeFunction implements ExchangeFunction
    {
        ...
        public Mono<ClientResponse> exchange(ClientRequest request)
        {
            ...
            return Mono.just(ClientResponse.create(response.status())
                    .header(HttpHeaders.CONTENT_TYPE, response.contentType()).body(response.body()).build());
        }
    }
