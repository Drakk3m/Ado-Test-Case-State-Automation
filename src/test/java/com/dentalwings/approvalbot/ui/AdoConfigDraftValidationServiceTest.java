...
class AdoConfigDraftValidationServiceTest {
    ...
    void validationResultModelDistinguishesStatuses() {
        ...
        assertThat(result.fields()).extracting(ConfigFieldValidation::status).containsExactly(
                ConfigValidationStatus.VALID, ConfigValidationStatus.WARNING, ConfigValidationStatus.ERROR,
    ...
    void fieldReferenceValidationDetectsMissingFieldFromAdoFieldList() {
        var discovery = new ApplicationLocalConfigServiceTest.FixedDiscovery(List.of("ADOnis 2.0 Test Project"),
                List.of("Test Case"), List.of("System.Title", "Custom.ApproverTech"),
    ...
    void workItemTypeValidationDetectsMissingTestCaseFromAdoTypeList() {
        var discovery = new ApplicationLocalConfigServiceTest.FixedDiscovery(List.of("ADOnis 2.0 Test Project"),
                List.of("Bug"), List.of("System.Title", "Custom.ApproverTech", "Custom.ApproverTest"),
                List.of("Design", "In Review", "Approval"), List.of("sme@example.test", "sqa@example.test"));
        var service = ApplicationLocalConfigServiceTest.validatingService(discovery);
    ...
    void userValidationDistinguishesUnresolvedUsers() {
        var discovery = new ApplicationLocalConfigServiceTest.FixedDiscovery(List.of("ADOnis 2.0 Test Project"),
                List.of("Test Case"), List.of("System.Title", "Custom.ApproverTech", "Custom.ApproverTest"),
                List.of("Design", "In Review", "Approval"), List.of("sme@example.test"));
        var service = ApplicationLocalConfigServiceTest.validatingService(discovery);
    ...
    void displayNameOnlyUserIsNotMarkedValid() {
        var service = ApplicationLocalConfigServiceTest
                .validatingService(ApplicationLocalConfigServiceTest.validDiscovery());
    ...
    void validDistinctFieldsRemainAccepted() {
        ...
        assertThat(result.fields()).filteredOn(field -> field.field().contains(".fields.")).noneSatisfy(
                field -> assertThat(field.message()).contains("duplicate", "must be different", "also be reversible"));
    }
