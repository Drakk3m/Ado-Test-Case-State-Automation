...
public class InMemoryProcessedEventStore implements ProcessedEventStore {
    ...
    public void markProcessed(ProcessedEventKey key, WorkItemProcessingResult result) {
        ...
        records.put(key, new ProcessedEventRecord(
                key,
                result.result(),
                retryCount(result.result()),
                nextRetryAt(result.result(), now),
                now,
                completedAt(result.result(), now),
                errorCategory(result.result()),
                result.reason()
        ));
    }
    ...
    public void cleanupExpired() {
        ...
        while (records.size() > maxRecords) {
            records.entrySet().stream()
                    .min(Comparator.comparing(entry -> entry.getValue().receivedAt()))
