...
final class ProjectIdentityCandidateCache {
    ...
    synchronized void put(String organization, String project, ProjectIdentityCandidates candidates) {
        ...
        projectReferences.put(
                new ProjectNameKey(normalized(organization), normalized(project)),
        ...
        while (entries.size() > maxEntries) {
            ...
            projectReferences.entrySet().removeIf(entry -> normalized(entry.getValue().projectId()).equals(removed.projectId()));
        }
    }
    ...
    record ProjectIdentityCandidates(
            boolean available,
            String projectId,
