...
final class IdentitySearchResultCache
{
    ...
    synchronized void put(String organization, String project, String query,
            ConfigLookupResult<ConfigSelectorOption> result)
    {
        var sanitizedValues = result.values().stream().map(option -> new ConfigSelectorOption(option.value(),
                        option.displayName(), option.description(), option.source(), "", option.avatarUrl(), option.resolved()))
                .toList();
    ...
    record CachedIdentitySearch(ConfigValidationStatus status, String message, List<ConfigSelectorOption> values)
    {
