...
public class AzureDevOpsConfigDiscoveryService implements AdoConfigDiscoveryService {
    ...
    private <T> ConfigLookupResult<String> readOne(Supplier<String> url, Class<T> responseType, Function<T, List<String>> mapper) {
        return read(url, responseType, mapper);
    }

    private <T> ConfigLookupResult<String> readList(Supplier<String> url, Class<T> responseType, Function<T, List<String>> mapper) {
        return read(url, responseType, mapper);
    }

    private <T> ConfigLookupResult<String> read(Supplier<String> url, Class<T> responseType, Function<T, List<String>> mapper) {
    ...
    private <T> Mono<ConfigLookupResult<String>> handleResponse(
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    record AdoRestFieldResponse(String referenceName) {
    }
