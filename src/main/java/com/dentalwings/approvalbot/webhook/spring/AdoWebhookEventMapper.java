...
public class AdoWebhookEventMapper
{
    ...
    public AdoWebhookEvent toWebhookEvent(AdoServiceHookWorkItemUpdatedRequest request)
    {
        ...
        var revisionFields = resource == null || resource.revision() == null ? Map.<String, Object> of()
                : resource.revision().fields();
        var changedBy = changedBy(resource, revisionFields);

        return AdoWebhookEvent.workItemUpdated(stringValue(request == null ? null : request.organization()),
                stringValue(fieldValue(
                        resource == null ? null : resource.project(), revisionFields.get(SYSTEM_TEAM_PROJECT))),
                resource == null ? null : resource.id(),
                stringValue(fieldValue(resource == null ? null : resource.workItemType(),
                        revisionFields.get(SYSTEM_WORK_ITEM_TYPE))),
                revision(resource), changedBy.displayName(), changedBy.emailOrLogin(), changedFieldNames(resource));
    }
    ...
    private IdentityValues changedBy(AdoServiceHookWorkItemUpdatedRequest.Resource resource,
            Map<String, Object> revisionFields)
    {
        var source = resource != null && resource.revisedBy() != null ? resource.revisedBy()
                : revisionFields.get(SYSTEM_CHANGED_BY);
        if (source instanceof AdoServiceHookWorkItemUpdatedRequest.Identity identity)
        {
            return new IdentityValues(identity.displayName(),
                    firstNonBlank(identity.uniqueName(), identity.email(), identity.mailAddress()));
        }
        if (source instanceof Map<?, ?> values)
        {
            return new IdentityValues(stringValue(values.get("displayName")),
