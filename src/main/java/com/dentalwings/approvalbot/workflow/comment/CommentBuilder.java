...
public class CommentBuilder {
    ...
    public Optional<String> fromDecision(WorkflowDecision decision) {
        if (decision.result() == ProcessingResult.SKIPPED || !decision.patchRequired() || decision.comment() == null || decision.comment().isBlank()) {
    ...
    public String proposedChanges(Identity changedBy, Collection<ChangedField> changedFields) {
        ...
        var identityLine = verifiedEmail
                ? "Proposed changes by %s [%s](mailto:%s).".formatted(displayName(changedBy), changedBy.email().trim(), changedBy.email().trim())
    ...
    private String changedFieldsText(Collection<ChangedField> changedFields) {
        return changedFields.stream()
                .sorted(Comparator.comparing(ChangedField::fieldReferenceName))
                .map(field -> """
                        * %s:
                          Previous:
                          %s
                          ...
                          Proposed:
                          %s""".formatted(field.fieldReferenceName(), renderValue(field.previousValue()), renderValue(field.proposedValue())))
