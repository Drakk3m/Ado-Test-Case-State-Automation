...
public class ProjectApprovalConfigValidator
{
    ...
    private void validateReversibleBusinessFields(ProjectApprovalConfig config,
            ArrayList<ConfigValidationIssue> issues)
    {
        ...
        var reversibleFields = config.reversibleBusinessFields().stream().map(this::normalizeField)
    ...
    private void validateCrossRoleWarnings(ProjectApprovalConfig config, ArrayList<ConfigValidationIssue> issues)
    {
        ...
        smeEmails.stream().filter(sqaEmails::contains).findAny().ifPresent(
                email -> issues.add(ConfigValidationIssue.warning("Same email appears in both SME and SQA lists.")));
    }
    ...
    private Set<String> validNormalizedEmails(Set<String> users)
    {
        ...
        return users.stream().flatMap(user -> emailNormalizer.normalize(user).stream()).filter(this::isValidEmail)
                .collect(Collectors.toUnmodifiableSet());
    }
