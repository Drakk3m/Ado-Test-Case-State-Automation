...
public class WorkItemProcessingService {
    ...
    public WorkItemProcessingResult process(ProcessWorkItemCommand command) {
        ...
        LOGGER.info("Workflow decision project={} workItemId={} revision={} result={} patchRequired={} reason={}",
                ...
                command.workItemKey().workItemId(),
                command.revision(),
                workflowDecision.result(),
                workflowDecision.patchRequired(),
                workflowDecision.reason());

        if (!workflowDecision.patchRequired()) {
            LOGGER.info("Patch skipped project={} workItemId={} revision={} reason={}",
                    command.workItemKey().project(),
                    command.workItemKey().workItemId(),
                    command.revision(),
        ...
        if (hasNoFieldOperations(patchOperations)) {
            LOGGER.info("Patch skipped project={} workItemId={} revision={} reason={}",
                    ...
                    command.workItemKey().workItemId(),
                    command.revision(),
                    "Patch contains no field operations.");
        ...
        LOGGER.info("Comment attempted project={} workItemId={} revision={}",
                command.workItemKey().project(),
                command.workItemKey().workItemId(),
                command.revision());
        var commentResult = adoClient.createWorkItemComment(command.workItemKey(), comment.get());
        if (!commentResult.successful()) {
            LOGGER.warn("Comment failed after successful patch project={} workItemId={} revision={} message={} outcome={}",
                    command.workItemKey().project(),
                    command.workItemKey().workItemId(),
                    command.revision(),
